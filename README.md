...
## Discussion
本研究の結果から、滑走記録アプリ間には、同一条件下で計測を行った場合でも、滑走距離や軌跡の精度に一定の差異が生じることが明らかとなった。特に、QGIS上でGPXデータを比較した結果、Stravaは一部にラグが見られたものの、他のアプリと比較して滑走軌跡が比較的細かく、実際に滑走したコース形状に近い形で記録されている傾向が確認された。一方で、Strava以外のアプリでの滑走データは、拡大して比較すると軌跡が簡略化されており、細かなターンやコース形状が十分に反映されていなかった。これは、各アプリごとのGPS取得間隔の違いが、記録精度に影響しているためであると考えられる。
